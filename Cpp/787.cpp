...
// marked blue in the picture. Note:
//
...
public:
    struct comp {
        bool operator()(vector<int> l, vector<int> r) { return l[0] > r[0]; }
    };
    ...
    int findCheapestPrice(int n,
        ...
        priority_queue<vector<int>, vector<vector<int>>, comp>
            q; // price, city, stops
        q.push({0, src, K});
        ...
        while (!q.empty()) {
            ...
            if (stops >= 0) {
                auto next_citys = m[city];
                for (auto next : next_citys) {
                    q.push({price + next.second, next.first, stops - 1});
                }
            }
        }
