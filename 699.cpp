...
// edge can stick to surfaces. Note:
//
...
public:
    ...
    vector<int> fallingSquares(vector<pair<int, int>>& positions) {
        ...
        for (auto p : positions) {
            Interval cur(p.first, p.first + p.second - 1, p.second);
            int h = getHeight(ints, cur);
            if (!res.empty() && h <= res.back()) {
    ...
    int getHeight(vector<Interval>& ints, Interval cur) {
        int prevHeight = 0;
        for (auto i : ints) {
            if (i.end < cur.start) {
                continue;
            ...
            prevHeight = max(prevHeight, i.height);
        }
        cur.height += prevHeight;
        ints.push_back(cur);
        return cur.height;
    }
};
