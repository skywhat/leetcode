...
public:
		...
		for(int step=1;step<=p;++step){
			for(int i=0;i<n;++i){
				for(int j=i;j<n;++j){
					...
					if(i!=j){
						dp[step][i][j]=max(dp[step][i][j],getValue(step-1,i-1,j,n,dp));
						dp[step][i][j]=max(dp[step][i][j],getValue(step-1,i-1,j-1,n,dp));
						dp[step][i][j]=max(dp[step][i][j],getValue(step-1,i,j-1,n,dp));
						dp[step][i][j]=max(dp[step][i][j],getValue(step-1,i,j,n,dp));
	...
	int getValue(int step,int x1,int x2,int n,vector<vector<vector<int>>>& dp){
