/*
input int[], 
Print out all unique indexes that satisfies nums[A] + nums[B] = nums[C] + nums[D]
Duplicate: (A,B) == (B,A), (C,D) == (D,C)
            (A,B,C,D) == (C,D,A,B) == (B,A,C,D)==(C,D,B,A)==(D,C,A,B)==(D,C,B,A).....
Example: [10,20,0,10,20]
          0  1  2 3 4
print:     0,1,3,4
           0,2,2,3
           1,2,2,4
			...
			for(int b=a+1;b<nums.size();++b){
				...
				for(int c=0;c<nums.size()-1;++c){
					for(int d=c+1;d<nums.size();++d){
						...
						if((b<d||(b==d&&a<c))&&nums[c]+nums[d]==sum){
		...
		for(auto&& it:m){
			for(int i=0;i<it.second.size()-1;++i){
				for(int j=i+1;j<it.second.size();++j){
					res.push_back({it.second[i].first,it.second[i].second,it.second[j].first,it.second[j].second});
				}
			}
		}
